...
public class XMLModifier {
    ...
    public void remove(long l1, long l2, short actionType) throws NavException, ModifyException{
    	...
    	if ((os1<os2 && os1+len1< os2) || (os2<os1 && os1+len1<os2)){
    		throw new ModifyException(" Non-overlapping segements");
    	}else if(os1<os2){
    		if (os1+len1 < os2+len2 ){
    			b=0;  //l1 overlaps l2
    		}else{
    			b=1; // l1 contains l2
    		}
    	}else{
    		if (os2+len2 < os1+len1){
    			b=2;  // l1 overlap l2
    		}else {
    			b=3;  // l2 contains l1
    		}
		...
		switch (actionType) {
		case SEGMENT_OP_UNION_STRICT:
		    switch(b){
		    case 0: l = ((long)(os2+len2-os1)<<32) | os1; break;
		    case 1: l = l1; break;
		    case 2: l = ((long)(os1+len1-os2)<<32) | os2; break;
		    case 3: l = l2;break;
		    }
		    remove(l);
		    return;
		case SEGMENT_OP_INTERSECT_STRICT:
			switch(b){
		    case 0: l =  ((long)(os1+len1-os2)<<32) | os2;break;
		    case 1: l = l2; break;
		    case 2: l =  ((long)(os2+len2-os1)<<32) | os1;break;
		    case 3: l = l1;break;
		    }
			remove(l);
			return;
		case SEGMENT_OP_EXCLUDE_STRICT:
			switch(b){
		    case 0: l =  (long)((os2-os1)<<32) | os1;remove(l);return;
		    case 1: remove(((long)(os2-os1)<<32) | os1); remove(((long)(os1+len1-(os2+len2))<<32)| (os2+len2)); return;
		    case 2: l =  (long)((os1-os2)<<32) | os2;remove(l);return;
		    case 3: remove(((long)(os1-os2)<<32) | os2); remove(((long)(os2+len2-(os1+len1))<<32)| (os1+len1)); return;
		    }
